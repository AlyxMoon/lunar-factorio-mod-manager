...
function ModManager (modListPath, modDirectoryPath, baseModPath, playerDataPath) {
  ...
  this.missingNecessaryDependencies = []
  this.missingOptionalDependencies = []
...
ModManager.prototype.getInstalledModByName = function (modName) {
  logger.log(0, `ModManager.getInstalledModByName() called with argument: ${modName}`)
  return this.installedMods.find(mod => {
      ...
      fs.readFile(`${this.modDirectoryPath}${files[i]}`, (error, rawZipBuffer) => {
        ...
        JSZip.loadAsync(rawZipBuffer).then((zip) => {
          ...
          if (counter <= 0) {
            ...
            return this.addMissingDependencies(callback)
          }
        })
      })
    }
...
ModManager.prototype.addMissingDependencies = function (callback) {
  if (typeof (callback) !== 'function') callback = function () {}

  this.missingOptionalDependencies = []
  this.missingNecessaryDependencies = []

  // Fix for mods that have a single dependency as a string
  this.installedMods = this.installedMods.map(mod => {
    if (typeof mod.dependencies === 'string') {
      mod.dependencies = [mod.dependencies]
    }
    let missingDependencies = mod.dependencies.filter(dependency => {
      dependency = dependency.trim()
      let optional = dependency[0] === '?'
      if (optional) dependency = dependency.substr(1).trim()
      ...
      let modDependency = this.installedMods.find(mod => mod.name === name)
      if (!modDependency) {
        if (optional) {
          this.missingOptionalDependencies.push(dependency)
          return false
        } else {
          this.missingNecessaryDependencies.push(dependency)
          return true
        }
      }
      let versionHigher = helpers.isVersionHigher(modDependency.version, version)
      if (versionHigher) {
        if (optional) {
          this.missingOptionalDependencies.push(dependency)
          return false
  ...
  // Remove duplicates
  this.missingNecessaryDependencies = this.missingNecessaryDependencies.reduce((a, b) => {
    if (a.indexOf(b) < 0) a.push(b)
    return a
  }, [])
  this.missingOptionalDependencies = this.missingOptionalDependencies.reduce((a, b) => {
    if (a.indexOf(b) < 0) a.push(b)
    return a
  }, [])
  ...
  function fetchFullOnlineMod (url, callback) {
    request(url, (error, response, data) => {
      if (!error && response.statusCode === 200) {
        ...
        // Doing it this way so that we don't have multiple of the same online mod
        // once we start polling it differently
        let index = self.onlineMods.findIndex(element => {
          return element.name === data.name
        })
        if (index === -1) {
          self.onlineMods.push(data)
        } else {
          self.onlineMods[index] = data
        }
...
// Queries a single online mod and adds that as a property to the corresponding installed mod
ModManager.prototype.addOnlineModInfoToInstalledMods = function (callback) {
  if (typeof (callback) !== 'function') callback = function () {}
  let apiURL = 'https://mods.factorio.com/api/mods'
  ...
  for (let i = 0; i < mods.length; i++) {
    request(`${apiURL}/${mods[i].name}`, (error, response, data) => {
      if (error) return callback(error)
      if (!error && response.statusCode === 200) {
        mods[i]['online_data'] = JSON.parse(data)
        count++

        if (count === mods.length - 1) return callback(null)
      }
    })
  }
}

// Intended to be run after 'addOnlineModInfoToInstalledMods()', adds another property with the latest available update if there is one
ModManager.prototype.addLatestAvailableUpdate = function (callback) {
  if (typeof (callback) !== 'function') callback = function () {}

  this.installedMods = this.installedMods.map(mod => {
    let onlineMod = mod.online_data
    let latestAvailableUpdate
    if (onlineMod) {
      latestAvailableUpdate = onlineMod.releases.find(release => {
        let versionHigher = helpers.isVersionHigher(mod.version, release.version)
        let sameFactorioVersion = mod.factorio_version === release.factorio_version
        return versionHigher && sameFactorioVersion
      })
    }

    if (latestAvailableUpdate) mod['latestAvailableUpdate'] = latestAvailableUpdate
    return mod
  })
...
ModManager.prototype.downloadMissingDependencies = function (callback) {
  if (typeof (callback) !== 'function') callback = function () {}
  if (this.missingNecessaryDependencies.length === 0) return callback(null)

  let apiURL = 'https://mods.factorio.com/api/mods'
  this.missingNecessaryDependencies.forEach(dependency => {
    if (dependency[0] === '?') dependency = dependency.substr(1).trim()
    let name
    let indexOfThingy = dependency.indexOf('>=')
    if (indexOfThingy === -1) {
      name = dependency.trim()
    ...
    request(`${apiURL}/${name}`, (error, response, data) => {
      if (error) callback(error)
      if (!error && response.statusCode === 200) {
        data = JSON.parse(data)
        let releaseToDownload = data.releases.find(releaseInfo => {
          return helpers.isSameFactorioVersion(releaseInfo.factorio_version, this.factorioVersion)
        })

        if (releaseToDownload) {
          callback(null, data.name, `https://mods.factorio.com${releaseToDownload.download_url}?username=${this.playerUsername}&token=${this.playerToken}`)
        }
      }
    })
  })
...
ModManager.prototype.getDownloadInfo = function (modName, downloadLink, callback) {
  ...
  for (let j = this.installedMods.length - 1; j >= 0; j--) {
    if (this.installedMods[j].name === modName) {
      modIndex = j
      break
    }
  }

  let fullLink = `https://mods.factorio.com${downloadLink}?username=${this.playerUsername}&token=${this.playerToken}`
  return callback(null, fullLink, modIndex)
}
