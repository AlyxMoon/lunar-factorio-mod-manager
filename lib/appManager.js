...
class AppManager {
  ...
  loadConfig(electronDialog, screenWidth, screenHeight) {
    return new Promise((resolve, reject) => {
      let data
  ...
  buildConfigFile(electron, screenWidth, screenHeight) {
    return new Promise((resolve, reject) => {
      // ------------------------------
  ...
  promptForPlayerDataPath(electron) {
  ...
  closeProgram(app, profileManager, inError = false) {
    if (inError) {
