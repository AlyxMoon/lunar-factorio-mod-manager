...
AppManager.prototype.loadConfig = function (electronDialog, screenWidth, screenHeight) {
  return new Promise((resolve, reject) => {
    ...
    storage.get('appManager', (error, file) => {
      if (error) {
        this.buildConfigFile(electronDialog, screenWidth, screenHeight)
      ...
      try {
        ...
        if (Object.keys(file).length === 0 && file.constructor === Object) {
          ...
          if (!data.hasOwnProperty('automatically_poll_online_mods')) {
            logger.log(3, 'automatically_poll_online_mods not in config file, setting to default')
            data.automatically_poll_online_mods = false
          }

          if (!data.hasOwnProperty('automatically_update_installed_mods')) {
            logger.log(3, 'automatically_update_installed_mods not in config file, setting to default')
            data.automatically_update_installed_mods = false
          }
          ...
          if (!data.hasOwnProperty('factorio_save_path') || typeof data.factorio_save_path !== 'string') {
            // Critical, can't fudge this one if not there
            logger.log(3, 'factorio_save_path not in config file, rebuilding config file.')
            this.buildConfigFile(electronDialog, screenWidth, screenHeight)
              .then(config => resolve(config)).catch(err => reject(err))
          }
...
AppManager.prototype.buildConfigFile = function (electron, screenWidth, screenHeight) {
  return new Promise((resolve, reject) => {
    ...
    let factorioSavePath = this.promptForFactorioSavePath(electron)
    if (factorioSavePath === undefined) {
      logger.log(4, 'User cancelled the dialog search.')
      return null
    }
    ...
    let data = {
      ...
      'factorio_save_path': factorioSavePath,
      'automatically_poll_online_mods': false,
      'automatically_update_installed_mods': false
    }
    ...
    storage.set('appManager', data, (error) => {
      if (error) {
        ...
        logger.log(1, `Saved appManager config file`)
        this.loadConfig(electron, screenWidth, screenHeight)
          .then(config => resolve(config)).catch(err => reject(err))
      }
    })
  })
...
AppManager.prototype.fetchLatestAppInfo = function () {
  ...
  request(options, (error, response, data) => {
    if (!error && response.statusCode === 200) {
      ...
      if (error) logger.log(3, `Error fetching latest app version. Error: ${error.message}`)
      if (response) logger.log(3, `Error fetching latest app version. Response: ${response.statusCode}`)
    }
  })
}
...
AppManager.prototype.promptForModlist = function (electron) {
  ...
  if (modlistPath) return modlistPath[0]
...
AppManager.prototype.promptForGamePath = function (electron) {
  ...
  let options = {
    ...
    'properties': ['openFile'],
    'filters': [{'name': 'Factorio Executable'}]
  }
...
AppManager.prototype.promptForFactorioSavePath = function (electron) {
  logger.log(1, 'Attempting to find Factorio saves directory.')
  ...
  switch (os.platform()) {
    case 'win32':
        // ------------------------------
        // Guess for common file locations first - Windows
      paths.push(path.join(electron.app.getPath('appData'), 'Factorio', 'saves'))
      paths.push(path.join('C:\\', 'Program Files', 'Factorio', 'saves'))
      paths.push(path.join('C:\\', 'Program Files (x86)', 'Factorio', 'saves'))

      break
    case 'linux':
        // ------------------------------
        // Guess for common file locations first - Linux
      paths.push(path.join(electron.app.getPath('home'), '.factorio', 'saves'))
      paths.push(path.join(electron.app.getPath('home'), 'factorio', 'saves'))
  ...
  for (let i = 0, length = paths.length; i < length; i++) {
    if (fs.existsSync(paths[i])) {
      logger.log(1, `Found factorio saves directory automatically: ${paths[i]}`)
      return paths[i]
    }
  }
    ...
    // ------------------------------
    // Prompt if we didn't find anything
  logger.log(1, 'Could not find automatically, prompting user for directory location.')
  let options = {
    'title': 'Find location of Factorio saves directory',
    'properties': ['openDirectory']
  }

  let factorioSavePath = electron.dialog.showOpenDialog(options)

  if (factorioSavePath) return factorioSavePath[0]
  else return undefined
}
