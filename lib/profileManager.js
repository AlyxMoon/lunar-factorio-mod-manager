...
ProfileManager.prototype.buildProfilesFile = function() {
  return new Promise((resolve, reject) => {
    ...
    let mods = JSON.parse(data, (key, value) => {
      if (value == 'true') {
        return true
      } else if (value == 'false') {
        return false
...
ProfileManager.prototype.updateFactorioModlist = function() {
  ...
  fs.writeFileSync(this.modlistPath, JSON.stringify(modList, (key, value) => {
...
ProfileManager.prototype.toggleMod = function(profileIndex, modIndex) {
  logger.log(1, `Attempting to toggle mod with index '${modIndex} in profile with index ${profileIndex}'`)

  // -------------------------------------------------------
  // New code. It seems to be working now

  if (this.profileList['active-profile'].mods[modIndex].enabled) {
