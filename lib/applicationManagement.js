...
AppManager.prototype.loadConfig = function(electronDialog, screenWidth, screenHeight) {
    ...
    try {
        ...
        if(error.code === 'ENOENT') {
            logger.log(2, 'Was not able to find config file. Creating.');
            return this.buildConfigFile(electronDialog, screenWidth, screenHeight);
        }
        else { // TODO: Handle unexpected errors more appropriately?
            logger.log(4, `Unhandled error attempting to load config file. Error: ${error.message}`);
            return null;
        }
    }
    ...
    try {
        ...
        logger.log(3, `Error parsing config file, rebuilding. Error: ${error.message}`);
        return this.buildConfigFile(electronDialog, screenWidth, screenHeight);
    }
...
AppManager.prototype.buildConfigFile = function(electron, screenWidth, screenHeight) {
    ...
    try {
        fs.writeFileSync(this.configPath, JSON.stringify(data, null, 4));
    }
...
AppManager.prototype.promptForModlist = function(electron) {
    ...
    for(let i = 0, length = windowsPaths.length; i < length; i++) {
        try {
            fs.readFileSync(windowsPaths[i], 'utf8');
            logger.log(1, `Found mod-list.json automatically: ${windowsPaths[i]}`);
...
AppManager.prototype.promptForGamePath = function(electron) {
    logger.log(1, 'Attempting to find Factorio.exe file.');
    ...
    for(let i = 0, length = windowsPaths.length; i < length; i++) {
        try {
            fs.readFileSync(windowsPaths[i], 'utf8');
            logger.log(1, `Found Factorio.exe automatically: ${windowsPaths[i]}`);
            return windowsPaths[i];
        }
        catch(error) { if(error.code !== 'ENOENT') return undefined; }
    }
...
AppManager.prototype.promptForPlayerDataPath = function(electron) {
    ...
    for(let i = 0, length = windowsPaths.length; i < length; i++) {
        try {
            fs.readFileSync(windowsPaths[i], 'utf8');
            logger.log(1, `Found player-data.json automatically: ${windowsPaths[i]}`);
            return windowsPaths[i];
        }
        catch(error) { if(error.code !== 'ENOENT') return undefined; }
    }
...
AppManager.prototype.loadPage = function(window, page, profileManager, modManager) {
    logger.log(1, `Attempting to change the page to ${page}`);
    ...
    switch(page) {
        ...
        case 'page_about':
            ...
            break;
        default:
            logger.log(3, 'The given page is not set up yet. Loading profiles page instead.');
